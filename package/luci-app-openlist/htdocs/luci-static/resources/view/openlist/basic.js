...
return view.extend({
	load: function() {
		return Promise.all([
			L.resolveDefault(callServiceList('openlist')),
			uci.load('openlist')
		]);
	},
	...
	render: function(data) {
		...
		var port = uci.get_first('openlist', 'openlist', 'port');
		var running = Object.keys(data[0].instances || {}).length > 0;
		var button, status;
		if (running) {
		    status = '<font color="green" size=3><b>' + _('OpenList is running') + '</b></font>';
		    button = '&#160;<a class="btn" href="http://' + window.location.hostname + ':' + port + '" target="_blank">' + _('Open Web Interface') + '</a>';}
		    else {
		    status = '<font color="red" size=3><b>' + _('OpenList is not running') + '</b></font>';
		    button = '';}
		...
		m = new form.Map('openlist', _('OpenList'),_('Default WebUI/WebDAV login username is %s and password is %s.').format('<code>admin</code>', '<code>admin</code>') +
			'<br><a href="https://oplist.org/zh/guide/" target="_blank">' +
			_('User Manual') );
		...
		o.rmempty = false;
		o = s.taboption('global', form.Value, 'site_url', _('Site URL'), _('When the web is reverse proxied to a subdirectory, this option must be filled out to ensure proper functioning of the web. Do not include \'/\' at the end of the URL'));
		...
		o.default = '48';
		o.rmempty = false;
		o = s.taboption('global', form.Value, 'max_connections', _('Max Connections'), _('0 is unlimited, It is recommend to set a low number of concurrency (10-20) for poor performance device'));
		o.default = '0';
		o.datatype = 'uinteger';
		o.rmempty = false;
		o = s.taboption('global', form.Value, 'max_concurrency', _('Max concurrency of local proxies'), _('0 is unlimited, Limit the maximum concurrency of local agents. The default value is 64'));
		o.default = '0';
		o.datatype = 'uinteger';
		o.rmempty = false;
		...
		o.rmempty = false;
		o.depends('ssl', '1');
		o = s.taboption('scheme', form.Value, 'ssl_cert', _('SSL cert'), _('SSL certificate file path'));
		o.rmempty = false;
		o.depends('ssl', '1');
		o = s.taboption('scheme', form.Value, 'ssl_key', _('SSL key'), _('SSL key file path'));
		o.rmempty = false;
		o.depends('ssl', '1');
